...
1. 数组查询(查询表的某几列) ```java //查询有阅读和唱歌爱好的人 BmobQuery

  <person> query = new BmobQuery<person>();
  String [] hobby = {"阅读","唱歌"};
  query.addWhereContainsAll("hobby", Arrays.asList(hobby));
  query.findObjects(new FindListener<person>() {</person></person></person>

  @Override public void done(List

  <person> object, BmobException e) {</person>
...
1. 增：添加数据 ```java //Person类加上private List

  <string> hobbys;<pre>

    <code>   //private List&lt;Bankcard&gt; card;
  </code>
  </pre></string>

//添加数据 Person p = new Person(); p.setObjectId("7a436bc5ef");
