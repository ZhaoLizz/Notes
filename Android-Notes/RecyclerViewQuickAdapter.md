...
- 创建实体类，implement MultiItemEntity，给每个数据设置一个itemType（用于在Adapter的构造里和layout绑定）

- 创建Adapter extends BaseMultiItemQuickAdapter

  <multipleitem,baseviewholder>，在构造里一一addItemType绑定TYPE和layout。  在convert中根据TYPE分别为子项赋值</multipleitem,baseviewholder>

- 创建DataServer类，写个方法为MultipleItem赋值

- 在主活动设置layoutManager，adapter
